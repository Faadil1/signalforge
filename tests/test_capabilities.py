...
def test_capability_contract_is_explicit_and_side_effect_free() -> None:
    ...
    assert body["state_model"]["refusal_recovery"] == "stateless_live_read_from_current_decision_packet"
    assert body["state_model"]["recovery_verification"] == "caller_supplied_refusal_plan_compared_to_fresh_live_packet"
    ...
    assert body["differentiators"]["refusal_recovery"]
    assert body["differentiators"]["recovery_verification"]
    ...
    assert "plan_evidence_recovery" in tool_names
    assert "verify_evidence_recovery" in tool_names
    ...
    assert body["safe_failure_contract"]["recovery_guarantees_actionability"] is False
    assert body["safe_failure_contract"]["recovery_verification_guarantees_correct_direction"] is False
...
def test_health_advertises_agent_contract_mcp_and_proof_surfaces() -> None:
    ...
    assert body["recovery_plan"] == "/api/v1/decision/{token}/recovery-plan"
    assert body["recovery_verification"] == "/api/v1/decision/{token}/verify-recovery"
