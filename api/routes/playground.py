...
def _build_endpoints(settings) -> list[dict]:
    endpoints = [
        {"method": "GET", "path": "/api/v1/signal/{token}", "description": "Live composite signal with explicit provenance and confidence gating"},
        {"method": "GET", "path": "/api/v1/decision/{token}", "description": "Evidence-bound Decision Packet for agent consumption"},
        {"method": "GET", "path": "/api/v1/decision/{token}/delta", "description": "Material-change delta vs the previous observed packet"},
        {"method": "GET", "path": "/api/v1/validation/{token}", "description": "Historical calibration of the price-derived signal subset"},
        {"method": "GET", "path": "/api/v1/signals", "description": "Live signals for multiple tokens (comma-separated)"},
        {"method": "GET", "path": "/api/v1/signal/{token}/history", "description": "Historical close/high/low/volume from Binance klines"},
    ...
    if settings.enable_backtests:
        ...
        endpoints.append({"method": "GET", "path": "/api/v1/strategy/{id}/backtest", "description": "Backtest a strategy on real Binance klines (experimental)"})
    if settings.enable_alerts:
        ...
        endpoints.append({"method": "POST", "path": "/api/v1/alerts/evaluate", "description": "Evaluate active alerts vs live signal, fire webhooks"})
    return endpoints
