...
from models.responses import ErrorDetail, HistoryResponse, MarketCards, RecommendationThresholdOut, SignalCard, SignalError, SignalMetaOut, SignalResponse, SignalsBatch, SignalsMetaResponse
...
def _card_from_payload(payload: dict) -> SignalCard:
    if payload["ok"]:
        return SignalCard(ok=True, token=payload["token"], price=payload["price"], score=payload["score"], confidence=payload["confidence"], recommendation=payload["recommendation"], actionability=payload.get("actionability"), data_mode=payload.get("data_mode"))
    return SignalCard(ok=False, token=payload["token"], error=ErrorDetail(code=payload["error"]["code"], message=payload["error"]["message"]))
...
async def get_signals_meta() -> SignalsMetaResponse:
    signals = [SignalMetaOut(key=key, name=SIGNAL_META[key]["name"], weight=SIGNAL_WEIGHTS[key], description=SIGNAL_META[key]["description"]) for key in SIGNAL_WEIGHTS]
    thresholds = [
    ...
    return SignalsMetaResponse(signal_count=len(signals), total_weight=SIGNAL_WEIGHTS_SUM, signals=signals, recommendation_thresholds=thresholds)
...
async def get_all_signals(tokens: str = Query(default=",".join(DEFAULT_TOKENS))) -> SignalsBatch:
    ...
    if not raw:
        raise HTTPException(status_code=422, detail=error_token_payload("", INVALID_TOKEN, "Provide at least one token"))
    normalized = list(dict.fromkeys(normalize_token(t) for t in raw))
    if len(normalized) > settings.max_batch_tokens:
        raise HTTPException(status_code=422, detail=error_token_payload(",".join(normalized), TOO_MANY_TOKENS, f"Too many tokens: {len(normalized)} (max {settings.max_batch_tokens})"))
...
async def get_signal_history(token: str, days: int = Query(default=30, ge=1, le=90)) -> HistoryResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=error_token_payload(symbol, SIGNAL_FETCH_FAILED, f"Upstream data unavailable: {exc}")) from exc
